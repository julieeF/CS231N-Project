...
def create_spectogram(input_vec, file_index, sess, noise_removal=0,truncated_wav_file_name=""):
                ...
                plt.savefig(iemocap_pre_processed_data_path + imf + truncated_wav_file_name + '.png')
                ...
                del Xdb
                del X
                plot_spectrogram1(truncated_wav_vector, "--", noise_removal, file_index, sess,truncated_wav_file_name)
                #time.sleep(1)
             
def plot_spectrogram1(signal, title, noise_removal, file_index, sess,truncated_wav_file_name):
    ...
    plt.savefig(iemocap_pre_processed_data_path + imf + truncated_wav_file_name + '_Same_ColorBAR.png')
    #truncated_wav_file_name
    plt.close('all')
    plt.close()
    print("FIG NUMBERS",plt.get_fignums())
    del signal
    ...
    for orig_wav_file in tqdm(base_name):
        ...
        try:
            ...
            for index, row in labels_df[labels_df['wav_file'].str.contains(orig_wav_file)].iterrows():
                ...
                aa = 0
                f_name = str(emotion) +"_"+ str(val) +"_"+ str(act) +"_"+ str(dom) + "_" + str(aa) + "_" + str(truncated_wav_file_name) + "_Whole_Length"
                create_spectogram(truncated_wav_vector, "_Whole_Length", sess,0, f_name)
